...
- **v0.1.119**：**严格遵守项目数据优先原则**（移除前端所有行业通用参考数据展示：手机端硬编码车间下拉列表改为从项目资料动态加载+手动输入；电脑端8个设备类型下拉框全部改为文本输入框，从项目资料识别或手动输入；隐藏矿山设备知识库展示区域，仅内部AI参考不在前端展示；替换所有placeholder中的球磨机/破碎机/浮选机等具体设备名示例为通用文本；新增/api/workshops端点返回当前项目已识别的车间列表；所有车间、设备、工艺流程均以用户上传的项目资料为准，随资料完善自动更新）
- **v0.1.118**：**手机端语音+文字+图片组合上传**（手机端新增语音录制功能，支持MediaRecorder录音，录制后可回放确认再上传；新增组合上传界面，语音、文字、图片可同时填写后一键同步上传；上传前显示组合状态指示（语音/文字/图片是否已准备）；图片支持多选和预览，可单独删除；电脑端新增/api/mobile/combined-upload端点，接收语音+文字+图片组合上传，自动保存临时文件并扫描解析入库；离线时文字自动保存到本地缓存）
- **v0.1.117**：**修复自动更新功能**（修复GitHub API频率限制导致检查更新失败的问题：改用raw.githubusercontent.com读取main.py版本号，无API频率限制，失败时回退到API方式；403错误时给出明确提示和手动下载链接；前端新增「检查更新」按钮，可手动触发版本检测；检测到新版本时弹出提示并显示更新横幅；优化错误提示信息）
- **v0.1.116**：**前端大幅精简**（将23个导航项+142个功能区块精简为7大模块Tab：项目管理、文件上传解析、解析结果、关系与空间、工程资料生成、平台规范库、云端与多机；文件上传类合并为一个统一上传窗口，包含扫描文件夹/手动上传/批量上传/失败重试四个快捷按钮，上传后自动解析分类；模块间通过顶部Tab和左侧导航一键切换，只显示当前模块内容；所有功能区块支持点击标题折叠/展开，默认精简显示；保留全部原有功能，只是界面更简洁）
- **v0.1.115**：**电脑端前端布局优化**（重构为左侧固定导航+右侧主内容区的现代桌面布局；左侧导航按功能分6大类：项目管理、文件上传、解析结果、关系与空间、工程资料、平台与云端，共23个导航项；点击导航平滑滚动定位到对应功能区；滚动时自动高亮当前所在区块；新增回到顶部按钮；移动端响应式适配，小屏幕自动隐藏侧边栏+汉堡菜单；主内容区最大宽度从980px扩展到1200px，内容更舒展；修复多个区块重复使用id=project-bar的问题，改为唯一ID；项目信息栏优化为渐变背景卡片样式）
- **v0.1.114**：**手机端代码完全分离**（将web/mobile/手机端网页从电脑端工作台中移除，创建独立的mobile_web手机端网页项目，支持配置任意电脑端API地址，可独立部署或直接浏览器打开；电脑端工作台不再包含任何手机端前端代码，安装包更纯净；Android APP v1.3.0改为从assets内置加载手机端网页，不再依赖电脑端的网页服务，只通过API与电脑端交互；电脑端新增CORS中间件允许手机端网页跨域访问API；手机端和电脑端完全解耦，各自独立迭代）
...
- **v0.1.103**：**矿山设备故障诊断AI助手（基于常见故障库）**（新增app/mining_fault_diagnosis.py；FAULT_DATABASE详细故障诊断库球磨机4种典型故障+高压釜4种典型故障每种含症状4-5条/原因6条/解决方案6条/预防措施5条/严重程度/紧急程度；球磨机5项监测参数含正常范围/报警阈值/停机阈值；高压釜5项监测参数；diagnose_fault基于症状匹配进行故障诊断计算匹配度排序输出可能故障列表+最可能故障+诊断建议；get_equipment_faults获取设备常见故障列表；get_fault_detail获取故障详细信息；get_monitoring_parameters获取监测参数；get_fault_diagnosis_stats故障诊断库统计；generate_fault_diagnosis_prompt生成故障诊断AI提示词含系统提示词和用户提示词；新增/api/mining-fault/{stats,equipment-faults,fault-detail,monitoring,diagnose,prompt}端点；前端新增故障诊断AI助手区）
- **v0.1.102**：**矿山设备吊装方案自动生成增强（基于详细参数）**（新增app/mining_lifting_plan_enhanced.py；基于设备详细参数和吊装参数库自动生成完整吊装方案；12个标准章节工程概况/编制依据/吊装设备参数/吊装方法选择/吊车选型及参数/吊点设计/索具配置/吊装顺序及工艺/吊装计算/安全技术措施/应急预案/人员组织；9种吊车参数库25t~500t含各作业半径额定载荷；5种索具参数库钢丝绳/合成吊带/卸扣/手拉葫芦/千斤顶；5种吊装方法单机/双机/滑移/回转/提升法；select_crane吊车选型考虑动载系数1.1和不均衡系数1.1计算载荷率和安全余量；select_rigging索具配置4点吊装60度角计算单根受力和安全系数；design_lifting_points吊点设计按设备类型专用吊耳；calculate_lifting吊装计算含计算公式；generate_lifting_plan生成完整方案；get_lifting_plan_template/get_crane_database/get_rigging_database查询接口；新增/api/mining-lifting-enhanced/{template,crane-database,rigging-database,generate}端点；前端新增吊装方案增强区）
- **v0.1.101**：**矿山设备施工方案自动生成增强（基于详细知识库）**（新增app/mining_construction_plan_enhanced.py；基于mining_equipment_detail设备详细知识库自动生成完整施工方案；11个标准章节工程概况/编制依据/施工准备/施工工艺流程/施工方法及技术要求/质量标准及验收要求/安全技术措施/应急预案/施工进度计划/资源配置计划/文明施工及环保措施；generate_construction_plan生成完整方案含设备参数/部件/施工要点/质量标准/验收项目自动填充；10条通用安全措施+6类设备专用安全措施（磨机/高压釜/炉/破碎/浮选/浓缩压滤）；6类通用应急预案含应急组织/联系人/应急物资；12阶段施工进度计划约25天含关键里程碑；10类人员配置+10类施工设备+6类材料配置；get_plan_template获取模板结构；get_available_equipment_for_plan获取可生成设备列表；新增/api/mining-plan-enhanced/{template,equipment-list,generate}端点；前端新增施工方案增强区）
- **v0.1.100**：**矿山设备知识库持续完善**（新增app/mining_equipment_detail.py；8种典型设备详细参数库球磨机/半自磨机/高压釜/闪速炉/浮选机/颚式破碎机/浓缩机/压滤机各含典型型号7-8种/关键参数8项/主要部件10-12项/施工要点6-8条/质量标准3-5项/常见故障3-4条含原因和解决方案/验收项目7-9项；EQUIPMENT_PHASE_POINTS分阶段施工要点球磨机4阶段（开箱/基础/安装/试运转）各含4-8条具体要点高压釜4阶段；get_equipment_detail获取设备详细信息支持精确和模糊匹配；get_all_equipment_details获取所有设备列表；get_equipment_construction_points按阶段筛选施工要点；get_equipment_quality_standards获取质量标准；get_equipment_common_faults获取常见故障含原因和解决方案；get_equipment_acceptance_items获取验收项目；get_equipment_phase_points获取分阶段施工要点；get_knowledge_base_stats知识库统计；新增/api/mining-equipment-detail/{list,detail,construction-points,quality-standards,common-faults,acceptance-items,phase-points,stats}端点；前端新增设备详细知识库区）
- **v0.1.99**：**矿山设备移动端APP界面优化**（新增web/mobile/index.html移动端专用界面26KB；移动端自适应布局viewport禁止缩放；底部Tab导航5个标签首页/上传/记录/AI/更多；首页项目概览卡片设备总数/资料总数/现场记录/待处理；8个快捷操作按钮拍照上传/语音记录/文字记录/记录列表/资料生成/AI助手/施工进度/更多；待办提醒列表；上传页文件选择+上传人+车间+备注；记录页全部/待解析/已解析/批量解析筛选；资料页8种记录类型生成；AI助手7种咨询模式；施工进度页8个阶段资料检查/清单/全进度/进度计划；更多页6个功能入口+关于信息；移动端触控友好按钮44px最小点击区域；新增/api/mobile/{config,dashboard}端点；前端主页面新增移动端入口链接）
- **v0.1.98**：**矿山设备多电脑并库现场记录合并**（新增app/mining_field_record_merge.py；merge_field_records合并现场记录支持多电脑并库；_calculate_record_md5基于内容MD5去重排除元数据字段；_find_conflict冲突检测同一记录类型+设备+日期；三种冲突策略latest用最新替换/keep_existing保留现有/manual待人工确认；get_merge_stats合并统计按记录类型/源节点/操作类型统计；get_pending_confirmations待人工确认列表；resolve_pending_confirmation处理待确认accept_incoming/keep_existing/merge_both；get_merge_log合并日志保留最近200条；check_field_record_integrity完整性检查必填字段/MD5补全；clear_merge_data清空数据带备份；新增/api/mining-field-merge/{merge,stats,pending,resolve,log,integrity}端点；前端新增多电脑并库现场记录合并区）
